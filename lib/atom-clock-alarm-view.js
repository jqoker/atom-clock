...
export default class AtomClockAlarmView {
  ...
  initialize() {
    ...
    this.subscriptions.add(atom.config.onDidChange('atom-clock-alarm.duration', () => {
      this.refreshTicker()
    }))
  ...
  setConfigValues() {
    ...
    this.duration = atom.config.get('atom-clock-alarm.duration') * 1000 * 60
  }
  ...
  setDate() {
    ...
    this.elapsed += this.refreshInterval

    if (this.elapsed / 60 >= this.duration) {
      this.elapsed = 0
